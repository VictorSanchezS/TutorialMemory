...
public class Juego extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        tvDatos = findViewById(R.id.usuario);

        Bundle recibeDatos = getIntent().getExtras();
        String info = recibeDatos.getString("keyNombre");
    ...
    private void comprobar(int i, final ImageButton imgb) {
        if (primero == null) {
            primero = imgb;
            configurarBotonImagen(primero, imagenes[arrayDesordenado.get(i)]);
            numeroPrimero = arrayDesordenado.get(i);
        } else {
            bloqueo = true;
            configurarBotonImagen(imgb, imagenes[arrayDesordenado.get(i)]);
            numeroSegundo = arrayDesordenado.get(i);
            if (numeroPrimero == numeroSegundo) {
                manejarEmparejamiento();
            } else {
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        reiniciarBotonesImagen(imgb);
                    }
                }, 1000);
            }
        }
    ...
    private void configurarBotonImagen(ImageButton imgButton, int recursoImagen) {
        imgButton.setScaleType(ImageView.ScaleType.CENTER_CROP);
        imgButton.setImageResource(recursoImagen);
        imgButton.setEnabled(false);
    }

    private void manejarEmparejamiento() {
        primero = null;
        bloqueo = false;
        aciertos++;
        puntuacion++;
        ...
        if (aciertos == imagenes.length) {
            ...
            Toast.makeText(getApplicationContext(), "Acertó", Toast.LENGTH_SHORT).show();
        }
    }

    private void reiniciarBotonesImagen(ImageButton imgb) {
        ...
        final Toast toast = Toast.makeText(getApplicationContext(), "Seguir intentando", Toast.LENGTH_SHORT);
        toast.show();

        // Cancela el toast después de medio segundo
